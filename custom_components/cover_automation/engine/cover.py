...
class CoverEngine:
    ...
    def evaluate(
        ...
        if not p.enabled:
            # The wind edge detector keeps tracking while disabled: an episode that was
            # active when the cover was disabled and released out of sight would otherwise
            # look like a release on the next enabled evaluation and open a restoring window
            # for wind that is long gone.
