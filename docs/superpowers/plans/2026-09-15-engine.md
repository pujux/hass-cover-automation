...
def test_graceful_holds_last_value_then_none():
    ...
    assert g.update(None, plus(30)) == "sunny"
    assert g.update(None, plus(59)) == "sunny"
    assert g.update(None, plus(60)) is None
...
def test_daily_latch_flag_latches_until_rollover():
    ...
    assert latch.update(date(2026, 7, 2), 20.0, 10.0, 24.0, 13.0) is False
...
def test_frost_signal_hysteresis_grace_and_unknown():
    ...
    assert f.update(None, plus(50)) is True  # grace: last value held
    assert f.update(None, plus(40 + 1800)) is None  # unknown after grace
    assert f.near_freezing_last_known is True
    f2 = FrostSignal(0.0, 1.0, 1800)
    f2.update(10.0, T0)
    f2.update(None, plus(1800))
...
def test_validate_rejects_bad_rules():
    ...
    assert timedelta(0) == timedelta()  # keep import used for readers extending this file
```
